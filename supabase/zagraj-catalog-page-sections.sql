-- Typed, audited page content. No executable code, arbitrary href or unsafe markup.
update public.zagraj_catalog_games set published=published||'{"pageSections":[]}'::jsonb
where not (published ? 'pageSections');
update public.zagraj_catalog_games set draft=draft||'{"pageSections":[]}'::jsonb
where draft is not null and not (draft ? 'pageSections');
...
begin
 ...
 for section in select value from jsonb_array_elements(raw_sections) loop
   if jsonb_typeof(section)<>'object'
      ...
      or section->>'kind' not in ('info','steps','notice')
