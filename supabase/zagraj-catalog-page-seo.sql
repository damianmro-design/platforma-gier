-- II.3d: Versioned SEO title/description for seven internal game pages, retaining editorial card copy.
...
begin
 ...
 -- Only these seven fixed, numbered card sets are supported. No rule title,
 ...
 if jsonb_typeof(page_rules) is distinct from 'null' then
   if p_slug not in ('va-banque','zakrecone-haslo','akta-nocy',
     'co-ludzie-powiedza','pod-przykrywka','szyfr','tylko-my')
   ...
   expected_rule_count:=case p_slug
     when 'va-banque' then 6
     when 'zakrecone-haslo' then 4
     when 'akta-nocy' then 4
     when 'co-ludzie-powiedza' then 9
     when 'pod-przykrywka' then 6
     when 'szyfr' then 3
     when 'tylko-my' then 4
     else null end;
  ...
  -- SEO belongs to an existing internal game. URLs, canonicals and robots are app-owned.
 if p_data ? 'pageSeo' then
   page_seo := p_data->'pageSeo';
 else
   page_seo := coalesce(g.draft->'pageSeo',g.published->'pageSeo','null'::jsonb);
 end if;
 if jsonb_typeof(page_seo) is distinct from 'null' then
   if p_slug not in ('akta-nocy','co-ludzie-powiedza','pod-przykrywka',
     'szyfr','tylko-my','va-banque','zakrecone-haslo') then
     raise exception 'INVALID_PAGE_SEO_GAME' using errcode='22023'; end if;
   if jsonb_typeof(page_seo) is distinct from 'object'
      or jsonb_typeof(page_seo->'title') is distinct from 'string'
      or jsonb_typeof(page_seo->'description') is distinct from 'string'
      or (select count(*) from jsonb_object_keys(page_seo))<>2 then
     raise exception 'INVALID_PAGE_SEO_SCHEMA' using errcode='22023'; end if;
   seo_title:=trim(page_seo->>'title');
   seo_description:=trim(page_seo->>'description');
   if length(seo_title) not between 1 and 70
      or length(seo_description) not between 1 and 180
      or (seo_title||seo_description) ~* '<|>|https?://|www\\.' then
     raise exception 'INVALID_PAGE_SEO_COPY' using errcode='22023'; end if;
   page_seo:=jsonb_build_object('title',seo_title,'description',seo_description);
 end if;
 ...
 c:=g.published || jsonb_build_object(
  ...
  'sortOrder',(p_data->>'sortOrder')::integer,'isVisible',(p_data->>'isVisible')::boolean,'pageSections',sections,'coverPath',cover_path,'pageIntro',page_intro,'pageRules',page_rules,'pageSeo',page_seo
 );
...
begin
  ...
  restored := h.payload || jsonb_build_object(
    ...
    'pageSeo', coalesce(h.payload->'pageSeo','null'::jsonb),
