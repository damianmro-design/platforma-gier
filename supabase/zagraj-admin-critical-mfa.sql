-- Require a verified MFA (AAL2) for sensitive owner actions.
-- Draft editing and all player flows continue with ordinary login.
CREATE OR REPLACE FUNCTION public.zagraj_admin_set_staff(p_email text, p_role text, p_permissions text[] DEFAULT '{}'::text[], p_game_slugs text[] DEFAULT '{}'::text[], p_active boolean DEFAULT true)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
CREATE OR REPLACE FUNCTION public.zagraj_catalog_publish(p_slug text, p_expected_revision integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
